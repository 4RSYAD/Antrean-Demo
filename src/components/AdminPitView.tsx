...
export const AdminPitView: React.FC<AdminPitViewProps> = ({
  ...
  const activeInPit = queues.find((q) => q.pit_id === selectedPit?.id && (q.status === 'washing' || q.status === 'waiting_payment'));
  ...
  const handleFinishWashing = (queueId: string) => {
    ...
    if (target) {
      ...
      if (target.email && onSendEmailNotification) {
        onSendEmailNotification(target.is_paid ? 'completed_paid' : 'wash_finished', target);
      }
    }
  };
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      {/* Pit Selection Bar */}
      <div className="bg-white dark:bg-[#0F121C] border border-slate-200 dark:border-[#23293D] p-4 sm:p-6 rounded-3xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xs">
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto">
          ...
          <select
            ...
            className="w-full md:w-auto bg-slate-100 dark:bg-[#161A28] border border-slate-300 dark:border-[#23293D] text-slate-900 dark:text-slate-100 text-xs font-bold rounded-2xl px-4 py-2.5 focus:outline-none focus:border-emerald-500 shadow-xs cursor-pointer"
          >
            {pits.map((p) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-[#0F121C] border border-slate-200 dark:border-[#23293D] p-5 sm:p-8 rounded-3xl space-y-5 shadow-xs flex flex-col justify-between">
          <div className="p-5 sm:p-8 bg-slate-50 dark:bg-[#161A28] rounded-3xl border border-slate-200 dark:border-[#23293D] space-y-4 text-center relative overflow-hidden">
            <div className="flex items-center justify-between text-xs font-mono font-bold uppercase tracking-widest text-emerald-700 dark:text-emerald-400">
              <span className="truncate">{selectedPit?.nama_pit.toUpperCase()}</span>
              <span
                className={`px-3 py-1 rounded-full text-[10px] font-bold font-mono whitespace-nowrap ${
                  activeInPit?.status === 'waiting_payment'
                    ...
                    : activeInPit
                    ? 'bg-teal-100 dark:bg-teal-950/60 text-teal-900 dark:text-teal-300 border border-teal-300 dark:border-teal-700 animate-pulse'
            ...
            {activeInPit ? (
              <div className="space-y-3 sm:space-y-4 py-2 sm:py-4">
                <div className="text-5xl sm:text-7xl lg:text-8xl font-black font-mono text-emerald-600 dark:text-emerald-400 tracking-tight">
                ...
                <div className="space-y-1.5">
                  ...
                  <div>
                    <span
                      className={`inline-flex items-center space-x-1 px-3 py-0.5 rounded-full text-xs font-bold ${
                ...
                <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-2xl bg-white dark:bg-[#0F121C] border border-slate-200 dark:border-[#23293D] text-xs">
              ...
              <div className="py-10 sm:py-14 text-center space-y-2.5">
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-3xl bg-slate-200 dark:bg-[#23293D] flex items-center justify-center mx-auto text-slate-500">
                  <Bike className="w-7 h-7 sm:w-8 sm:h-8" />
                </div>
                <div className="text-slate-600 dark:text-slate-400 text-xs sm:text-sm font-semibold max-w-xs mx-auto">
                  Pit Bay ini kosong. Panggil antrean berikutnya untuk mulai mencuci.
                </div>
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5 pt-2">
            <button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold py-3 px-3 rounded-2xl transition shadow-xs text-xs flex items-center justify-center space-x-1.5 cursor-pointer"
            >
            ...
            {activeInPit && (
              <>
                <button
                  ...
                  className="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black py-3 px-3 rounded-2xl transition shadow-xs text-xs flex items-center justify-center space-x-1.5 cursor-pointer"
                >
                ...
                {activeInPit.email && onSendEmailNotification && (
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-500 text-white font-black py-3 px-3 rounded-2xl transition shadow-xs text-xs flex items-center justify-center space-x-1.5 cursor-pointer"
                ...
                <button
                  ...
                  className="bg-orange-600 hover:bg-orange-500 text-white font-black py-3 px-3 rounded-2xl transition shadow-xs text-xs flex items-center justify-center space-x-1.5 cursor-pointer"
                >
        ...
        {/* Next In Line Column */}
        <div className="lg:col-span-4 bg-white dark:bg-[#0F121C] border border-slate-200 dark:border-[#23293D] p-5 sm:p-6 rounded-3xl space-y-4 shadow-xs flex flex-col justify-between">
                ...
                waitingQueues.map((q) => {
                  ...
                  return (
                    <div
                      ...
                      className="p-3 bg-slate-50 dark:bg-[#161A28] rounded-2xl border border-slate-200 dark:border-[#23293D] flex items-center justify-between text-xs hover:border-emerald-500/40 transition"
                    >
                      ...
                      <div className="flex items-center space-x-1 shrink-0">
                        ...
                        <button
                          ...
                          className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold rounded-xl text-[10px] transition flex items-center space-x-1 cursor-pointer"
          ...
          <div className="p-3 bg-slate-50 dark:bg-[#161A28] rounded-2xl border border-slate-200 dark:border-[#23293D] text-[11px] text-slate-600 dark:text-slate-400 font-medium">
            💡 Alur: Setelah cuci selesai, klik <b>Selesai Cuci</b> untuk memanggil pelanggan ke kasir.
          </div>
        </div>
      </div>
    </div>
